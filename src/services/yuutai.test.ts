...
describe('yuutaiService', () => {
  ...
  it('getYuutaiByTickerAndShares: 指定した株数のデータを返す', async () => {
    vi.mocked(repository.getYuutaiByTickerAndShares).mockResolvedValue([mockRows[0]]);

    const result = await service.getYuutaiByTickerAndShares(mockDb, '1301', 100);

    expect(result).toHaveLength(1);
    expect(result[0].shares_condition_num).toBe(100);
    expect(repository.getYuutaiByTickerAndShares).toHaveBeenCalledWith(mockDb, '1301', 100);
  });
