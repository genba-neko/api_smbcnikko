...
export const getYuutaiByTicker = async (db: D1Database, tickerCode: string): Promise<YuutaiResponse[]> => {
...
export const getYuutaiByTickerAndShares = async (
  ...
  const rows = await repository.getYuutaiByTickerAndShares(db, tickerCode, shares);
  return rows.map(mapRowToResponse);
};

export const getYuutaiByMonth = async (db: D1Database, recordDate: string): Promise<YuutaiResponse[]> => {
